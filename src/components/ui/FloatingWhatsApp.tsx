...
export function FloatingWhatsApp() {
  ...
  useEffect(() => {
    // Cek apakah consent cookie sudah tersimpan di awal
    const consent = localStorage.getItem("maroa_cookie_consent");
    if (!consent) {
      // Ada kemungkinan banner cookie muncul
    ...
    const handleCookieVisibility = (e: Event) => {
      ...
      if (customEvent.detail) {
        setIsCookieVisible(Boolean(customEvent.detail.visible));
      }
    };

    window.addEventListener("maroa_cookie_visibility", handleCookieVisibility);
    return () => {
      window.removeEventListener("maroa_cookie_visibility", handleCookieVisibility);
    };
  }, []);

  // Di layar mobile (< sm), jika banner cookie sedang tampil, tombol WA naik agar tidak menutupi tombol persetujuan
  const bottomPositionClass = isCookieVisible
    ? "bottom-52 sm:bottom-6"
    : "bottom-6";

  // Sembunyikan floating WhatsApp secara mutlak pada antarmuka admin/CMS
  ...
  return (
    <div
      className={`fixed ${bottomPositionClass} right-4 sm:right-6 z-50 flex flex-col items-end gap-3 font-sans transition-all duration-300`}
    >
          ...
          <div className="p-4 bg-gray-50 text-xs space-y-3">
            ...
            <div className="space-y-2 pt-1">
              <a
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold shadow-md transition-transform active:scale-95 cursor-pointer"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold shadow-sm transition-transform active:scale-95 cursor-pointer"
              >
      ...
      <div className="relative group">
        <button
          ...
          className="flex items-center gap-2 px-4 py-3 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 border-2 border-white/20 cursor-pointer"
        >
